    ...
    class Lokalise2Action < Action
      def self.run(params)
        ...
        if languages.kind_of? Array then
          request_data["filter_langs"] = languages.to_json
        end
        ...
        if response.kind_of?(Net::HTTPOK) && jsonResponse["bundle_url"].kind_of?(String) then
        ...
        elsif jsonResponse.has_key?("error")
